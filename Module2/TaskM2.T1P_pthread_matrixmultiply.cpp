...
void p_PopulateMatrix(int** matrix, int seed, int totalSize, int partitionSize, pthread_t arr[])
{
	for (size_t i = 0; i < (NUM_THREADS / 2); i++)
	{
		...
		PopTask -> end = (i + 1) == (NUM_THREADS / 2) ? totalSize : ((i + 1) * partitionSize);
...
void p_MultiplyMatrices(int** matrix1, int** matrix2, int** matrix3, int totalSize, int partitionSize, pthread_t arr[])
{
	for (size_t i = 0; i < NUM_THREADS; i++)
	{
		...
		// Set the end index of the partition for the thread, ensuring if it's the last row it returns the size of the full matrix size
		MulTask -> end = (i + 1) == NUM_THREADS ? totalSize : ((i + 1) * partitionSize);
...
int main()
{
	...
	pthread_t threads_popTask[NUM_THREADS];
	// Create thread array for matrix multiplication
	pthread_t threads_mulTask[NUM_THREADS];
	...
	int partitionSize = matrixSize / (NUM_THREADS / 2);

	// Populate first two with random variables
	p_PopulateMatrix(m1, 0, matrixSize, partitionSize, threads_popTask);
	p_PopulateMatrix(m2, NUM_THREADS, matrixSize, partitionSize, threads_popTask);
	...
	partitionSize = matrixSize / NUM_THREADS;

	// Multiply first two to produce third matrix
	p_MultiplyMatrices(m1, m2, m3, matrixSize, partitionSize, threads_mulTask);
